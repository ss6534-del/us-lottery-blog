...
// 훅은 각 앱의 "실제 앱 화면" 기준으로 작성했다(랜딩 테마가 아니라 — app4는 랜딩은
// 다크지만 실제 앱은 파스텔 "Lucky Day"처럼 실제와 다른 케이스가 있어, 스크린샷이
// 붙는 트윗에서는 실제 기능으로 소개해야 어긋나지 않는다). 금칙어(win/odds/guaranteed/
// jackpot) 없음, entertainment 톤. 앱당 2~3개를 시드로 회전.
export const APPS = [
...
const BASE_TAGS = ["#LotteryApp", "#Android"];
const GENERIC_TAGS = ["#Lottery", "#NumberGenerator", "#LotteryStats", "#AndroidApp"];
...
export function promoLink(app) {
  const base = String(app.promoUrl || `https://appfactory${String(app.id).replace(/^app/, "")}.ssgpost.org/`).replace(/\/+$/, "") + "/";
