...
export async function runSocial(newItems, state, opts = {}) {
  // 킬 스위치 — 모든 게이트·자격증명·API 호출보다 먼저. 여기서 멈추면 트윗도
  // 큐 적립도 과금도 0이고, false를 돌려주어 글 발행(state 저장)에는 영향이 없다.
  if (SITE.socialEnabled === false) {
    console.log("[social] disabled (site.config socialEnabled=false) — X 자동 트윗 전면 중단");
    return false;
  }
