...
const COLD_BAR = "#9BD1FF"; // 핫(accent 노랑)과 대비되는 차가운 하늘색 — 콜드 차트 전용

/**
 * 핫/콜드 결합 차트 — 스레드 트윗 2번(분석 A) 전용 이미지.
 * 위 절반 = 핫 8(빈도), 아래 절반 = 콜드 8(최저 빈도). post/analysis 둘 다
 * stats.hot / stats.cold 가 같은 모양이라 한 함수로 처리한다.
 */
function vizHotCold(j) {
  const hot = Array.isArray(j?.stats?.hot) ? j.stats.hot.slice(0, 8) : [];
  const cold = Array.isArray(j?.stats?.cold) ? j.stats.cold.slice(0, 8) : [];
  if (hot.length < 3 || cold.length < 3) return null;
  const win = j?.stats?.window;
  return {
    mode: "hotcold",
    kicker: "HOT & COLD",
    labelDate: j?.stats?.latest?.date || j?.publishedDate || null,
    hot: {
      title: `HOT · MOST DRAWN${win ? ` · LAST ${win} DRAWS` : ""}`,
      bars: hot.map((h) => ({ label: h.num, value: h.count })),
    },
    cold: {
      title: `COLD · LEAST DRAWN${win ? ` · LAST ${win} DRAWS` : ""}`,
      bars: cold.map((c) => ({ label: c.num, value: c.count })),
    },
  };
}
...
function dataOverlaySvg(viz, big, small) {
  ...
  } else if (viz.mode === "hotcold") {
    // 스레드용 핫/콜드 이중 차트 — 위 핫(accent), 아래 콜드(하늘색)
    body += `\n  <text x="${PAD}" y="150" font-size="27" font-weight="700" letter-spacing="2" fill="rgba(255,255,255,0.85)">${escXml(viz.hot.title)}</text>`;
    body += `\n  ${barsSvg({ x: PAD, w: innerW, baseY: 296, bars: viz.hot.bars, maxH: 96, color: ACCENT, barMax: 78 })}`;
    body += `\n  <text x="${PAD}" y="394" font-size="27" font-weight="700" letter-spacing="2" fill="rgba(255,255,255,0.85)">${escXml(viz.cold.title)}</text>`;
    body += `\n  ${barsSvg({ x: PAD, w: innerW, baseY: 536, bars: viz.cold.bars, maxH: 96, color: COLD_BAR, barMax: 78 })}`;
 ...
 */
/**
 * 글 JSON 파일 경로를 외부(social.js)에 노출 — 스레드 본문이 prose를 직접 읽어야 한다.
 * dataFileFor의 규칙(=build.js의 data/ 규칙)을 한곳에 유지하기 위한 재수출.
 */
export function postDataFile(kind, gameSlug, date) {
  return dataFileFor(kind, gameSlug, date);
}

// 스레드 트윗별 이미지 역할 — social.js의 buildThread가 이 이름으로 요청한다.
//   lead    : 대표 이미지(블로그 og:image와 동일 viz)
//   hotcold : 핫/콜드 이중 차트 (트윗 2 — 분석 A)
//   sets    : 예측 세트 카드 (post 전용, 트윗 3)
//   overdue : 장기 미출현 차트 (트윗 3)
const THREAD_VIZ = {
  hotcold: (j) => vizHotCold(j),
  sets: (j, game) => vizPostSets(j, game),
  overdue: (j) => vizPostOverdue(j),
};

/**
 * 스레드 트윗용 단일 역할 이미지 info — role: "lead"|"hotcold"|"sets"|"overdue".
 * 데이터가 없어 해당 viz를 못 만들면 null(호출자가 그 트윗을 텍스트-only로 보낸다).
 * 시드에 role을 붙여 장끼리 배경 풀이 달라진다(ogInfoSet의 "#N" 시드와 같은 원리).
 */
export function ogThreadInfo(item, role) {
  const base = ogInfo(item);
  if (role === "lead") return base;
  const build = THREAD_VIZ[role];
  if (!build) return null;
  const file = dataFileFor(item.kind, item.gameSlug, item.date);
  if (!file || !fs.existsSync(file)) return null;
  const j = readJsonSafe(file);
  if (!j) return null;
  let viz = null;
  try {
    viz = build(j, gameBySlug(item.gameSlug));
  } catch (e) {
    console.warn(`[og] thread viz ${role} failed for ${item?.kind}/${item?.gameSlug}/${item?.date}: ${e.message}`);
  }
  if (!viz) return null;
  return {
    ...base,
    file: base.file.replace(/\.jpg$/i, `-${role}.jpg`),
    seed: `${base.seed}#${role}`,
    viz,
  };
}
...
/**
 * info 1건을 임의 경로(임시 파일)로 합성 — social.js의 스레드 이미지용.
 * composePostImage와 같은 폴백 규칙(sharp 부재/실패 시 원본 복사).
 * @returns {Promise<boolean>} 파일이 만들어졌는지
 */
export async function composeInfoImage(info, outFile) {
  const pool = loadOgPool();
  if (pool.length === 0) {
    console.warn(`[og] no image pool — cannot compose ${outFile}`);
    return false;
