...
let NAV_COUNTS = { games: {}, digests: 0, recaps: 0, analysis: 0, news: 0 };
...
export function postCard(item) {
  ...
  const showImage = item.showImage === true && Boolean(item.ogImageFile);
  const thumb = showImage
    ? `<div class="a-thumb a-thumb--image">
    <img src="${u(`/og/${item.ogImageFile}`)}" alt="" width="${OG_W}" height="${OG_H}" loading="lazy" decoding="async">
  </div>`
  ...
  return `
<a class="a-card${showImage ? " a-card--image" : ""}" href="${u(item.path)}">
  ${thumb}
  <div class="a-body">
...
<aside class="drawer" id="drawer" aria-hidden="true" aria-label="Site menu">
  ...
  <nav>
    ...
    <a href="${u("/news/")}">${doodle("squiggle", 16)} Number Watch — draw oddities${cnt(NAV_COUNTS.news)}</a>
...
// ── 이변 탐지 뉴스 본문 (kind:"news") ────────────────────────────────────────

/** 이변 종류별 "무엇이 화제인가" 섹션 제목 */
const NEWS_HEAD = {
  "long-absence": "The number that stopped showing up",
  "repeat-number": "The numbers that came straight back",
  "parity-extreme": "The line with no balance",
  "sum-extreme": "The total that broke the band",
  "consecutive-pair": "The pairs that landed together",
};

/**
 * 이변의 주인공 번호들 — 화제의 번호만 골라 크게 보여준다.
 * 전체 당첨 라인은 위 wash-block이 이미 보여주므로 여기선 "왜 화제인가"만 남긴다.
 */
function newsFocus(post, key) {
  const nums = Array.isArray(post.facts.numbers) ? post.facts.numbers : [];
  if (nums.length === 0) return "";
  const f = post.facts;
  const caption = {
    "long-absence": `Last seen ${f.gapText || f.gap} draws ago — the longest absence in the ${post.newsStats.window}-draw window.`,
    "repeat-number": `Carried over from the ${f.prevDate ? longDate(f.prevDate) : "previous"} drawing.`,
    "parity-extreme": `All ${f.c} numbers came in ${f.parity}.`,
    "sum-extreme": `These add up to ${f.sum} — the ${f.dir} total of the window.`,
    "consecutive-pair": `${f.count} consecutive pairs: ${Array.isArray(f.pairs) ? f.pairs.join(" and ") : ""}.`,
  }[post.newsType];
  return `
  <div class="sketch-card" style="${gv(key)}">
    ${ballRow(nums, null, key, "l")}
    ${caption ? `<p class="ball-caption" style="margin:12px 0 0">${esc(caption)}</p>` : ""}
  </div>`;
}

/** 이변별 뒷받침 시각화 — 미출현 랭킹 / 합계 추이 / 빈도 막대 */
function newsEvidence(post, key) {
  const s = post.newsStats;
  if (post.newsType === "long-absence") {
    const rows = s.overdue
      ...
      .join("");
    return `
  ${sectionHead("The rest of the absence table", `${s.window}-draw window`)}
  <div class="sketch-card" style="${gv(key)}">${rows}</div>`;
  }
  if (post.newsType === "sum-extreme" && post.sumSeries && post.sumSeries.length > 1) {
    return `
  ${sectionHead("Line totals, recent draws")}
  <p>Each point is one drawing's total. The typical band for this window sits between <strong>${s.sum.p25}</strong> and <strong>${s.sum.p75}</strong> — the latest total sits outside it.</p>
  <div class="sketch-card" style="${gv(key)}">${sparkline(post.sumSeries, `var(--game-${key})`)}</div>`;
  }
  return `
  ${sectionHead("What's running hot right now")}
  <div class="stat-panel" style="${gv(key)}">
    <div class="ph"><span class="gdot"></span><h3>Most-drawn numbers · last ${s.window} draws</h3></div>
    ${crayonBars(s.hot.slice(0, 8).map((h) => ({ label: h.num, value: h.count })))}
  </div>`;
}

export function renderNewsBody(post, ogImageFile = null) {
  const game = gameById(post.gameId);
  const key = gameKey(post.gameId);
...
<article class="post">
  <div class="post-meta-row">
    ${gameTag(key, game.name)}
    ${sticker("Number Watch", -3)}
    <span class="mut">Published ${esc(longDate(post.publishedDate))} · flagged automatically from the ${post.newsStats.window}-draw window</span>
  </div>
  <h1>${esc(post.title)}</h1>
  ${leadImage(ogImageFile)}
  ...
  <div class="wash-block" style="${gv(key)}">
    <div class="lab">The line in question · ${esc(longDateWithDay(post.resultDate))}</div>
    ${ballRow(post.latest.white, post.latest.special, key, "l")}
    ${game.specialName ? `<p class="ball-caption">The colored ball is the ${esc(game.specialName)}.</p>` : ""}
  </div>

  ${sectionHead(NEWS_HEAD[post.newsType] || "What the tracker saw")}
  ${newsFocus(post, key)}
  <p>${esc(post.prose.fact)}</p>

  ${newsEvidence(post, key)}

  ${sectionHead("Before you read anything into it")}
  <div class="sketch-card">
    <p style="margin:0">${esc(post.prose.context)}</p>
  </div>

  ${stickyNote("Want the next result the second it lands?", "The free app posts every New York draw as it happens — and checks your saved tickets for you.", "Get the free app")}

  <p>See the full picture in our <a href="${u(`/${game.slug}/`)}">latest ${esc(game.name)} analysis</a> — hot and cold tables, pattern stats and AI-generated sets for the next drawing.</p>
  ...
  ${installPanel("Every draw, watched for you.")}
</article>`;
}
