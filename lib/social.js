...
import {
  buildPromoTweet, readPromoShot, promoBanned,
...
export async function runSocial(newItems, state, opts = {}) {
  ...
  // 3a) 앱 홍보 슬롯인가 — 화/금 저녁 슬롯을 앱 홍보로 대체(주 2회). 통계 글 큐와 별개.
  // 저녁 슬롯이 앱 홍보로 소진되면 그날 통계 글은 그 슬롯에서 안 나가지만,
  // 아침 통계 슬롯은 그대로다(= 하루 2건: 아침 통계 + 저녁 앱 홍보).
  const week = isoWeekKey(today);
  const promoState = normalizePromoState(state._promo, week);
  const promoDec = decidePromo(promoState, {
    slot: slot || effectiveSlot,
    weekday: etWeekday(today),
    promoForce,
  });
  if (promoDec.fire) {
...
async function firePromo(state, social, promoState, ctx) {
  ...
  if (dryRun) {
    ...
    console.log(
      `[social][DRY RUN][PROMO] ${hour}:00 ET (slot: ${effectiveSlot})\n` +
        ...
        `[social][DRY RUN][PROMO] image: ${shot ? `assets/app-shots/${app.shot}` : "none (text only — set app.shot)"}\n` +
        `----- promo tweet -----\n${tweet.text}\n-----------------------`
    );
  ...
  // 이미지 — 저장소에 넣어 둔 앱 스크린샷 jpg를 그대로 업로드(실패해도 텍스트 트윗은 나간다).
  let mediaId = null;
  try {
    const shot = readPromoShot(app);
    if (shot) {
      const r = await uploadMedia(shot.buf, creds);
      mediaId = r.mediaId;
      if (!mediaId) console.warn("[social][promo] media upload unavailable — tweeting text only");
    } else {
      console.warn(`[social][promo] no screenshot for ${app.id} (app.shot empty) — tweeting text only`);
    }
  } catch (err) {
